...
var metrics = {};
var blockNumber = 0, block, block2, block_hash = '';
...
var blockNumber = chain3.mc.blockNumber;
while ( ! block ) {
...
wsc.on('open', () => {
	console.log("[feed]...feeding data...");
	wsc.send(JSON.stringify(metrics));
});

wsc.on('close', () => console.log("[feed]about to close"));

wsc.on('message', function (data) {
	//console.log(`received ${data}`);
});

wsc.on('error', () => console.log("[feed]error happend, closing ..."))
